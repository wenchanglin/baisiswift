...
- (void)setupContentView{
    ...
    contentView.frame = CGRectMake(10, 64 + 10, self.view.width - 2 * 10, ScreenH);
...
- (void)textFieldContentDidChange{
    if (_textField.hasText) {
        _addBtn.hidden = NO;
        _addBtn.y = CGRectGetMaxY(self.textField.frame) + 5;
...
- (void)updateTagButtonAndTextFieldFrame{
    for (NSInteger i = 0; i < _tagBtns.count; i++ ) {
        ...
        if (i == 0) {
            ...
            CGFloat rightWidth = self.contentV.width - leftWidth - 5;
            if (rightWidth >= tagBtn.width) {
                tagBtn.y = lastTagBtn.y;
                tagBtn.x = leftWidth;
            }else{
                tagBtn.x = 0;
                tagBtn.y = CGRectGetMaxY(lastTagBtn.frame) + 5;
            }
        }
    }
    ...
    CGFloat leftWidth = CGRectGetMaxX(lastTagBtn.frame) + 5;
    if (self.contentV.width - leftWidth -5 >= [self textFieldWidth]) {
