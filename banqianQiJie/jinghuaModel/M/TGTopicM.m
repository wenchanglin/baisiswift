...
- (CGFloat)cellHeight{
    ...
    _cellHeight += [self.text boundingRectWithSize:textMaxSize options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName : [UIFont systemFontOfSize:14]} context:nil].size.height + 10;
    ...
    if (self.top_cmt.content.length > 0){//评论
        ...
        */
        ...
        textMaxSize = CGSizeMake(ScreenW - 2 * 10 - 10, MAXFLOAT);//再减10是评论区域内的左右边距
        ...
        textMaxSize = CGSizeMake(ScreenW - 2 * 10 - 10, MAXFLOAT);//再减10是评论区域内的左右边距
        _commentBtnX = [cmtText boundingRectWithSize:textMaxSize options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName : [UIFont systemFontOfSize:12]} context:nil].size.width + 10;
